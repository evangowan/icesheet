...
mapproject << END    ${R_options} ${J_options} -F >> ss_parameters.txt
...
grdimage ${nc_file} ${shift_up} -X3 -R${x_min}/${x_max}/${y_min}/${y_max}  -JX${map_width}/0 -K -P -Cshades_shearstress.cpt -V -nb > ${plot}
...
psscale -X-5 -Y-3.5 -Dx9c/2c/9c/0.5ch -P -O -Bx100000f20000+l"Shear Stress (Pa)" --FONT_LABEL=14p -Cshades_shearstress.cpt -V  >> $plot
